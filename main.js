...
axios
  .get("https://fizal.me/pokeapi/api/v2/name/gengar.json")
...
axios
  .get("https://fizal.me/pokeapi/api/v2/name/primeape.json")
  .then(function (answer) {
    console.log(answer);
    let name = answer.data.name;
...
axios
  .get("https://fizal.me/pokeapi/api/v2/name/parasect.json")
  .then(function (answer) {
    console.log(answer);
    let name = answer.data.name;
    let sprite = answer.data.sprites.front_default;
...
let createPB = function (p, p2, p3) {
  ...
  if (is_all_pb_complete) {
    // if true, then clear timeout
...
function typeEffect(element, speed) {
  ...
  var timer = setInterval(function () {
    if (i < text.length) {
      ...
      let trainerMsg = document.getElementById("centeralign");
      trainerMsg.style.display = "none";
      trainerImage.src =
        "https://vignette.wikia.nocookie.net/fcoc-vs-battles/images/a/ae/BE0CE5A2-6C6B-4FC6-91ED-AFAE016013A8.png/revision/latest?cb=20190307201518";
    }
  }, speed);
}
